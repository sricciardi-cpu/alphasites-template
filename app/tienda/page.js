...
// Icon map: JSON stores the icon name, this maps to actual React components
const ICON_MAP = { FaHeart, FaStar, FaShippingFast };

const valores = tenant.quienes_somos.valores.map((v) => ({
  icon: ICON_MAP[v.icono] ?? FaHeart,
  titulo: v.titulo,
  texto: v.texto,
}));
...
// Base de "vendidas" (marketing) — las ventas reales se suman a este número.
const VENDIDAS_BASE = tenant.stats.vendidas_base;
const MODELOS_FALLBACK = tenant.stats.modelos_fallback;
  ...
  return (
    <main>
      ...
      <section
        ...
        style={{ backgroundImage: `url('${tenant.hero.imagen}')`, backgroundSize: "cover", backgroundPosition: "center" }}
      >
        ...
        <div className="relative z-10 flex flex-col items-center pt-24 md:pt-32 pb-10 md:pb-16 min-h-[300px] md:min-h-[400px] justify-center w-full">
          <h1 className="text-3xl md:text-5xl font-extrabold mb-4 leading-tight">{tenant.hero.titulo}</h1>
          <p className="text-sm md:text-xl text-orange-500 font-semibold mb-10 max-w-xs md:max-w-none">
            {tenant.hero.subtitulo}
          </p>
          <Link
            href={tenant.hero.cta_href}
          ...
          <div className="relative z-10 w-full border-t border-white/20">
            <div className="max-w-5xl mx-auto px-4 py-8 grid grid-cols-2 gap-4 text-center">
              <div>
                ...
                <p className="text-xs md:text-sm text-gray-300 mt-1">{tenant.stats.vendidas_label}</p>
              </div>
              <div>
                <p className="text-2xl md:text-3xl font-extrabold text-orange-500">
                  <AnimatedCounter target={modelosCount} />
                </p>
                <p className="text-xs md:text-sm text-gray-300 mt-1">{tenant.stats.modelos_label}</p>
              </div>
            </div>
          </div>
        </FadeIn>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10 md:py-12">
        ...
        <FadeIn>
          <section className="bg-zinc-900 rounded-2xl text-white p-6 md:p-10 mb-12 md:mb-16 transition-transform duration-300 hover:-translate-y-1 border border-zinc-700">
            <h2 className="text-2xl md:text-3xl font-extrabold mb-4">{tenant.quienes_somos.titulo}</h2>
            <p className="text-gray-300 mb-8 max-w-2xl text-sm md:text-base leading-relaxed">
              {tenant.quienes_somos.texto}
            </p>
